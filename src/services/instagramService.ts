...
export const fetchInstagramPostByUrl = async (url: string): Promise<InstagramVideo | null> => {
  ...
  try {
    ...
    // Response is an array: [{ urls: [...], meta: {...}, pictureUrl: "..." }]
    const item = Array.isArray(data) ? data[0] : data;
    if (!item || !item.meta) throw new Error('Post not found or is private.');
    ...
    const views = likes * 10; // Instagram doesn't expose view count via this endpoint, estimate from likes
    ...
    return {
      ...
      channelId: meta.username || 'unknown',
      thumbnailUrl,
      caption: meta.title || '',
      views,
      engagement,
      outlierScore: 1,
      postedAt: meta.takenAt ? new Date(meta.takenAt * 1000).toISOString() : new Date().toISOString(),
      platform: 'Instagram',
      videoUrl,
      status: 'idle',
      transcript: '',
